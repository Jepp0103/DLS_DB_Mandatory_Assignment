...
-- -----------------------------------------------------
-- Placeholder table for view `roll_call_db`.`class_course`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `roll_call_db`.`class_course` (`classname` INT, `coursename` INT, `classid` INT, `courseid` INT, `teacherid` INT);
SHOW WARNINGS;
...
-- -----------------------------------------------------
-- View `roll_call_db`.`class_course`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `roll_call_db`.`class_course`;
SHOW WARNINGS;
USE `roll_call_db`;
CREATE 
 OR REPLACE VIEW `class_course` AS
    SELECT 
        `c`.`name` AS `classname`,
    ...
    FROM
        ((((`teacher_lectures` `tl`
        LEFT JOIN `lecture` `l` ON ((`tl`.`lecture_id` = `l`.`id`)))
        LEFT JOIN `class_lectures` `cl` ON ((`l`.`id` = `cl`.`lecture_id`)))
        LEFT JOIN `class` `c` ON ((`cl`.`class_id` = `c`.`id`)))
        LEFT JOIN `course` `co` ON ((`l`.`course_id` = `co`.`id`)))
    GROUP BY `tl`.`teacher_id` , `c`.`id` , `co`.`id`;
SHOW WARNINGS;
USE `roll_call_db`;
